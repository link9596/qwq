(function() {
    if (window._funnyCounterCoreLoaded) return;
    window._funnyCounterCoreLoaded = true;
    const API_BASE = 'https://funny-api.969608.xyz';
    const COMMIT_DELAY = 2000;
    const MAX_DELTA = 666;
    const MAX_PENDING = 666;
    ...
    async function commitDelta(delta) {
        ...
        try {
            ...
            if (data.count !== undefined) {
                ...
                if (isCountMode && !hintTimer && !limitHintTimer) setTextWithAnimation(`被摸了${currentCount}次`);
                console.log(`已提交${delta}，当前总数:${currentCount}`);
                return true
            }
            throw new Error('提交失败');
        } catch (err) {
            console.error('提交数量失败', err);
            return false
        }
    }
    ...
    function showLimitHint() {
        ...
        limitHintTimer = setTimeout(() => {
            if (originalMode) setTextWithAnimation(`被摸了${currentCount}次`);
            else setTextWithAnimation('暗中观察（被发现了）');
            limitHintTimer = null
        }, 1500)
    }
    ...
    function triggerComboAnimation() {
        ...
        const steps = [{
            ...
            transform: `translateX(${translateX*0.5}px)rotate(${rotate*0.5}deg)`,
            offset: 0.6
        }, {
    ...
    function endComboStreak() {
        if (currentComboStreak >= 10) showHintMessage(`x ${currentComboStreak}！`);
    ...
    function recordCombo() {
        if (comboTimer) clearTimeout(comboTimer);
        comboTimer = setTimeout(() => endComboStreak(), COMBO_RESET_DELAY)
    }
    ...
    function onMouseDown(e) {
        if (e.button === 0 && funnyCard) funnyCard.style.cursor = 'grabbing'
    }
    ...
    window.addEventListener('beforeunload', () => {
        if (pendingDelta > 0) {
            const data = JSON.stringify({
                delta: Math.min(pendingDelta, MAX_DELTA)
            });
            navigator.sendBeacon(`${API_BASE}/count`, new Blob([data], {
    ...
    function init() {
        ...
        fetchGlobalCount().then(count => {
            ...
            setTextWithAnimation('暗中观察（被发现了）');
            isCountMode = false;
            console.log('滑稽计数器，启动！当前总数：', currentCount);
        }).catch(err => {
